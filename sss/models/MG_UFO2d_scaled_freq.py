...
#     def forward(self, x):
#         return self.conv2(F.leaky_relu(self.conv1(x), negative_slope=0.1)) + self.shortcut(x)   
...
class Conv2D_Periodic(nn.Module):
    ...
    def forward(self, x, mult_tensor=None, freq = None):
...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x, mult_tensor=None, freq=None):
        ...
        # freq_ratio = torch.tensor([self.ref_freq / freq[0]], dtype=torch.float32, device=x.device)
        # freq_ratio = freq_ratio[None,None,:].repeat(self.weights.shape[0], self.weights.shape[1], 1)
        # weights_plus_freq = torch.cat((self.weights, freq_ratio), dim=-1)
        weights_plus_freq = self.weights

        weights = self.mlp(weights_plus_freq).reshape((self.in_channels, self.out_channels, self.modes1, self.modes2, 2))
...
class residual_blocks(nn.Module):
    ...
    def forward(self, x, mult_tensor=None, freq = None):
        for i in range(self.num_blocks):
            x1 = self.convs[i](x, mult_tensor, freq)
...
class DownSizedNetwork(nn.Module):
    ...
    def forward(self, x, coeff_tensors=None, freq = None):
        ...
        for level, (block, downsample) in enumerate(self.downs):
            ...
            x = block(x, coeff_tensor, freq)
            h.append(x)
            x = downsample(x)

        coeff_tensor = None if coeff_tensors is None else coeff_tensors[-1]
        x = self.mid_block(x, coeff_tensor, freq)
        h.append(x)
...
class UpSizedNetwork(nn.Module):
    ...
    def forward(self, x, h, coeff_tensors=None, freq = None):
...
@gin.configurable
class MGUFO2d_scaled_freq(nn.Module):
    """
    ...
    def setup(self, eps, freq):
        ...
        coeff_tensors = self.setup_net(eps, freq=freq)
    ...
    def forward(self, rhs, freq):
        ...
        h = self.solve_down_net(rhs, self.coeff_tensors, freq)
        output = self.solve_up_net(h[-1], h[:-1], self.coeff_tensors, freq)
