...
class NonUniformSchwarzRoll:
    ...
    Added rolling of domains, such that the subdomain interfaces within PML are reduced (only perpendicular no tangential)
    -------------------------------------------------
    |                     |                     |   |
    |                     |                     |   |
    |      regular        |       regular       |PML| # PML on the left is rolled over
    |      domain         |       domain        |   |
    |                     |                     |   |
    |                     |                     |   |
    |-----------------------------------------------|
    ...
    |                     |                     |   |
    |-----------------------------------------------|
    |         PML         |         PML         |PML| # PML on the top is rolled over
    -------------------------------------------------
    """
    ...
    def init_grid(self, pml_thickness):
        ...
        if pml_x > 0:
            self.roll_x = pml_x + self.extra_pml_pixels
            ...
            # last two min_overlaps for last subdomain in the center, and PML in the end
            x_overlaps = x_overlaps + [self.min_overlap, self.min_overlap]
            rows = center_rows + 1
        else:
            self.roll_x = 0
        ...
        if pml_y > 0:
            self.roll_y = pml_y + self.extra_pml_pixels
            ...
            y_overlaps = y_overlaps + [self.min_overlap, self.min_overlap]
            cols = center_cols + 1
        else:
            self.roll_y = 0
    ...
    def init_regions(self):
        ...
        for i in range(num_rows):
            col_start = 0
            if self.pml_thickness[0] > 0 and i == num_rows-1:
                sx = 2*self.pml_thickness[0] + 2*self.extra_pml_pixels + self.x_overlaps[-2] + self.x_overlaps[-1]
            else:
                sx = self.region_size[0]
            for j in range(num_cols):
                # Calculate slices for the current region
                if self.pml_thickness[1] > 0 and j == num_cols-1:
                    sy = 2*self.pml_thickness[1] + 2*self.extra_pml_pixels + self.y_overlaps[-2] + self.y_overlaps[-1]
                else:
                ...
                if self.pml_thickness[0] > 0 and i == num_rows-1 or self.pml_thickness[1] > 0 and j == num_cols-1:
    ...
    def partition(self, A):
        A_shape = A.shape
        ##### First roll the A such that PMLs are not on both sides, but on one side
        assert len(A_shape) == 2
        if self.roll_x > 0:
            A = torch.roll(A, shifts=-self.roll_x, dims=0)
        if self.roll_y > 0:
            A = torch.roll(A, shifts=-self.roll_y, dims=1)

        ##### Then circularly pad the A with wrapped around pixels
    ...
    def combine(self, x, x_pml, scale_with_POU=True):
        ...
        res =  reconstructed[:self.unpadded_shape[0], :self.unpadded_shape[1]]

        # now roll back the results to the original configuration
        if self.roll_x > 0:
            res = torch.roll(res, shifts=self.roll_x, dims=0)
        if self.roll_y > 0:
            res = torch.roll(res, shifts=self.roll_y, dims=1)
        return res
