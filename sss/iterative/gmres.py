...
class GMRES:
    ...
    def solve(self, b, verbose=False, max_iter=None, tol=None, return_xr_history=False):
        ...
        if verbose:
            pbar = tqdm(range(max_iter), desc="GMRES iteration", leave=False)
        else:
            pbar = range(max_iter)  
        for j in pbar:
            time1 = time.time()
            ...
            e1[:, 0] = beta
            regularizer = 0.0*torch.max(torch.abs(H[:, :num_iter + 1, :num_iter]))*torch.eye(num_iter + 1, dtype=torch.complex64, device=b.device)
            ...
            relres_history.append(r_ratio)

            # Check for convergence
            if verbose:
                print("Iteration: %d, Residual norm: %e, Relative residual norm: %e" % (num_iter, torch.abs(residual_norm), r_ratio))
